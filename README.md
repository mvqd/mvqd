# Hi, I'm Vuong 👋

## 🌐 Connect with me

🔗 LinkedIn: https://www.linkedin.com/in/qdmvuong/

📧 Email: [minhvuongqd@gmail.com](mailto:minhvuongqd@gmail.com)

---

Embedded Software Engineer with 3 years of experience in firmware and embedded systems.
I enjoy working with low-level systems, automation tools, and embedded Linux environments.

---

## 🔧 Skills

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bash/bash-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/matlab/matlab-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/gitlab/gitlab-original.svg" width="45" height="45"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="45" height="45"/>
</p>

---

## 🎯 Current Goals

* Build an embedded systems portfolio
* Document my learning journey
* Develop useful embedded and automation tools
* Explore remote opportunities in embedded software engineering

---

## 📚 Learning Progress

I plan to document my learning in areas such as:

* Embedded Linux
* RTOS
* Firmware architecture
* System programming
* Automation tools

Repositories will be added as I progress.

---

## 📂 Future Projects

Some projects I plan to work on:

* Embedded data logger (C/C++)
* Linux automation scripts (Bash)
* Firmware utility tools (Python)
* Control system simulations (MATLAB / Simulink)
